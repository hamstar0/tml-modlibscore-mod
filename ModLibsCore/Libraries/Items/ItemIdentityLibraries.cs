...
namespace ModLibsCore.Libraries.Items {
	/// <summary>
	...
	public partial class ItemIdentityLibraries {
		...
		public static int GetVanillaSnapshotHash( Item item, bool noContext, bool minimal ) {
			...
			if( !minimal ) {
				...
				hash += ( "summon" + item.CountsAsClass(DamageClass.Summon) ).GetHashCode() * Pow();
				hash += ( "thrown" + item.CountsAsClass(DamageClass.Throwing) ).GetHashCode() * Pow();
				hash += ( "ranged" + item.CountsAsClass(DamageClass.Ranged) ).GetHashCode() * Pow();
				hash += ( "magic" + item.CountsAsClass(DamageClass.Magic) ).GetHashCode() * Pow();
				hash += ( "melee" + item.CountsAsClass(DamageClass.Melee) ).GetHashCode() * Pow();
